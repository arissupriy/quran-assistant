...
#[frb]
pub async fn get_mushaf_page_context_info(page_number: i32) -> anyhow::Result<MushafPageInfo> { // Menggunakan anyhow::Result
    info!("Mendapatkan info kontekstual untuk halaman Mushaf: {}", page_number);

    let engine_data = &GLOBAL_DATA;
    let pack_state_guard = PACK_STATE.lock().unwrap();
    let mushaf_bundle = pack_state_guard.as_ref().context("Mushaf pack belum di-load. Panggil open_mushaf_pack terlebih dahulu.")?;

    let mut surah_name_arabic = "Tidak Diketahui".to_string();
    let mut juz_number = 0;
    let mut next_page_route_text = "".to_string();

    let current_glyphs: Option<&Vec<GlyphPosition>> = mushaf_bundle.index.pages.get(&(page_number as u16)).map(|entry| &entry.glyphs);

    if let Some(glyphs) = current_glyphs {
        if let Some(first_glyph_on_page) = glyphs.iter().min_by_key(|&g| (g.line_number as u32, g.word_position as u32)) { // Konversi ke u32
            ...
            if let Some(chapter_details) = engine_data.chapters.chapters.iter().find(|c| c.id == current_surah_id_from_glyph) {
                surah_name_arabic = chapter_details.name_arabic.clone();
            } else {
                warn!("Detail chapter tidak ditemukan di GLOBAL_DATA untuk surah ID: {}", current_surah_id_from_glyph);
            }
            ...
            if let Some(chapter_verses) = engine_data.verses_by_chapter.get(&current_surah_id_from_glyph) {
                if let Some(current_verse) = chapter_verses.iter().find(|v| v.verse_number == current_ayah_number) {
                    juz_number = current_verse.juz_number; // Langsung ambil u32
                } else {
                    warn!("Ayat {} tidak ditemukan di GLOBAL_DATA untuk surah {}.", current_ayah_number, current_surah_id_from_glyph);
                }
            } else {
                warn!("Chapter {} tidak ditemukan di GLOBAL_DATA.verses_by_chapter.", current_surah_id_from_glyph);
            }
        } else {
            warn!("Tidak ada glyph yang valid ditemukan di halaman {}.", page_number);
        }
    } else {
        warn!("Metadata glyph tidak ditemukan di PACK_STATE untuk halaman {}.", page_number);
    }

    let next_page_number = page_number + 1; // page_number input adalah i32
    let next_glyphs: Option<&Vec<GlyphPosition>> = mushaf_bundle.index.pages.get(&(next_page_number as u16)).map(|entry| &entry.glyphs);

    if let Some(glyphs) = next_glyphs {
        if !glyphs.is_empty() {
            let first_glyph_on_next_page_option = glyphs.iter().min_by_key(|&g| (g.line_number as u32, g.word_position as u32)); // Konversi ke u32

            if let Some(glyph) = first_glyph_on_next_page_option {
                if (glyph.page_number as i32) == next_page_number { // page_number di GlyphPosition adalah u16, next_page_number adalah i32
                    let next_glyph_sura: u32 = glyph.sura as u32;
                    let next_glyph_ayah: u32 = glyph.ayah as u32;

                    if let Some(next_chapter_verses) = engine_data.verses_by_chapter.get(&next_glyph_sura) {
                        if let Some(next_verse) = next_chapter_verses.iter().find(|v| v.verse_number == next_glyph_ayah) {
                            if !next_verse.words.is_empty() {
                                next_page_route_text = next_verse.words[0].text_uthmani.clone();
                                if next_verse.words.len() > 1 {
                                    next_page_route_text += &format!(" {}", next_verse.words[1].text_uthmani);
                                }
                            } else {
                                warn!("Ayat berikutnya kosong kata untuk halaman {}.", next_page_number);
                            }
                        } else {
        ...
        warn!("Metadata glyph tidak ditemukan di PACK_STATE untuk halaman berikutnya {}.", next_page_number);
    }

    Ok(MushafPageInfo {
        surah_name_arabic,
        juz_number,
        page_number: page_number as u32, // Sesuaikan dengan tipe yang diharapkan oleh model
