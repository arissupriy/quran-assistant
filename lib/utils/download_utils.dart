...
class MushafDownloadPage extends StatefulWidget {
  final int? initialPage; // Parameter: Halaman awal yang akan ditampilkan setelah unduhan
...
class _MushafDownloadPageState extends State<MushafDownloadPage> {
  ...
  Future<void> _startDownloadAndDecompress({
    ...
    String _dialogStatusMessage = "Mengunduh file mushaf...";
    
    late StateSetter _setDialogState; // Deklarasikan StateSetter

    // Tampilkan dialog yang tidak bisa ditutup
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (dialogContext) { // Ganti nama parameter context menjadi dialogContext
        return StatefulBuilder( // StatefulBuilder digunakan untuk mengelola state di dalam dialog
          builder: (context, setInnerState) { // setInnerState adalah setState() khusus untuk builder ini
            _setDialogState = setInnerState; // Simpan reference setInnerState
            return AlertDialog(
              title: const Text('Mempersiapkan Data'),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(_dialogStatusMessage), // Gunakan _dialogStatusMessage
                  const SizedBox(height: 16),
                  LinearProgressIndicator(value: _dialogProgress), // Gunakan _dialogProgress
                  const SizedBox(height: 8),
                  Text(_dialogProgress != null ? '${(_dialogProgress! * 100).toStringAsFixed(1)}%' : 'Mohon tunggu...'),
                ],
              ),
            );
          },
    ...
    try {
      ...
      await dio.download(
        ...
        onReceiveProgress: (received, total) {
          if (total != -1 && mounted) {
            ...
            _setDialogState(() {
              _dialogProgress = received / total;
            });
          }
        },
        options: Options( // Pastikan options ini ada jika Anda memilikinya di downloadWithDio
      ...
      if (mounted) {
        ...
        _setDialogState(() {
          _dialogStatusMessage = "Mengekstrak file...";
          _dialogProgress = null; // Buat progress bar menjadi indeterminate
        });
      }
